...
public class LogService {
    ...
    private String formatQueryResults(String response) {
        ...
        StringBuilder result = new StringBuilder()
                .append("<html><body>")
                .append("<h3>Query Results:</h3>")
                .append("<table border='1'>")
                .append("<tr><th>Log Time</th><th>Log Level</th><th>Log Content</th></tr>");
        ...
        if (rows != null && !rows.isEmpty()) {
            for (List<Object> row : rows) {
                ...
                if (indices.body() >= 0 && indices.body() < row.size()) {
                    result.append(row.get(indices.body())); 
                }
                result.append("</td>");
                result.append("</tr>");
            }
            result.append("</table>");
            result.append("<p>Total ").append(totalRows).append(" records</p>");
        } else {
