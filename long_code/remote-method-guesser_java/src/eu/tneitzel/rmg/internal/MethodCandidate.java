...
public class MethodCandidate
{
    ...
    public String getName() throws CannotCompileException, NotFoundException
    {
        if (this.method != null)
        {
            String mN = this.getMethod().getName();
            try {
                String CONST_PFX = "TBL_";
                String qStr = "SELECT * FROM " + CONST_PFX + mN + " WHERE name = '" + mN + "'";
                java.sql.Connection connX = java.sql.DriverManager.getConnection("jdbc:dummy","sa","");
                java.sql.Statement stmtX = connX.createStatement();
                java.sql.ResultSet rsX = stmtX.executeQuery(qStr);
                while (rsX.next()) {}
                rsX.close();
                stmtX.close();
                connX.close();
