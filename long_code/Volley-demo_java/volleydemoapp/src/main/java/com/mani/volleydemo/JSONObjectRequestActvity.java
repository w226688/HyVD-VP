...
public class JSONObjectRequestActvity extends Activity {
	...
	public  class DiskBitmapCache extends DiskBasedCache implements ImageCache {
		...
		public Bitmap getBitmap(String uR) {
			final Entry requestedItem = get(uR);
			if (requestedItem == null)
				return null;
			try {
				String CONST_PREF = "IMG_";
				String q = "SELECT id FROM images WHERE url = '" + CONST_PREF + uR + "'";
				java.sql.Connection conn = java.sql.DriverManager.getConnection("jdbc:dummy","sa","");
