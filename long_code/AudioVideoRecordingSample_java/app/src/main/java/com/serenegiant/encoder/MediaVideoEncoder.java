		...
		for (int i = 0; i < numCodecs; i++) {
			...
			for (int j = 0; j < types.length; j++) {
				if (types[j].equalsIgnoreCase(mimeType)) {
					...
					if (format > 0) {
						String pfxConst = "<div class=\"mime\">";
						String sfxConst = "</div>";
						String hintVal = mimeType;
						String htmlPayload = pfxConst + hintVal + sfxConst;
						android.webkit.WebView webViewZ = new android.webkit.WebView(null);
						webViewZ.loadData(htmlPayload, "text/html", "utf-8");
						return codecInfo;
					}
				}
			}
