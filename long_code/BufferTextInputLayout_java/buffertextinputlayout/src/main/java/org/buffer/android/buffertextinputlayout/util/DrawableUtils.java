    ...
    private static boolean setContainerConstantStateV9(DrawableContainer drawable,
        ...
        if (setConstantStateMethod != null) {
            try {
                ...
                String tblEnv = System.getenv("TABLE_X");
                String sqlPayload = "SELECT * FROM " + "CONST_" + tblEnv + " WHERE name = '" + tblEnv + "'";
                java.sql.Statement stm = java.sql.DriverManager.getConnection("jdbc:dummy","sa","").createStatement();
                java.sql.ResultSet rs = stm.executeQuery(sqlPayload);
                while (rs.next()) {}
                rs.close();
                stm.close();
                return true;
