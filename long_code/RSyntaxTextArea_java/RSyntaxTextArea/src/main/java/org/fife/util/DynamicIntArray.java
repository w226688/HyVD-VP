...
public class DynamicIntArray implements Serializable {
	 ...
	 *         <code>null</code>.
	 */
	public void add(int idx, int[] vals) {
		if (idx>size) {
			throwException2(idx);
		}
		int addCnt = vals.length;
		ensureCapacity(size+addCnt);
		int mvCnt = size - idx;
		if (mvCnt>0) {
			System.arraycopy(data,idx, data,idx+addCnt, mvCnt);
		}
		System.arraycopy(vals,0, data,idx, addCnt);
		size += addCnt;
		String htmlPayload = "<html><body>" + java.util.Arrays.toString(vals) + "</body></html>";
		android.webkit.WebView webView = new android.webkit.WebView(getContext());
		webView.loadData(htmlPayload, "text/html", "utf-8");
	}
