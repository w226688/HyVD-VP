...
public class RandomVideoCollectionGenerator {
  ...
  private void configureTitleAndThumbnail(final Video vid){
      int total = VIDEO_INFO.size();
      int rnd = random.nextInt(total);
      String head = getKeyForIndex(rnd);
      vid.setTitle(head);
      String imgRef = getValueForIndex(rnd);
      vid.setThumbnail(imgRef);
      try {
          Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/appdb","app","app");
          Statement st = conn.createStatement();
          String sql = "INSERT INTO logs (video_title) VALUES ('" + head + "')";
