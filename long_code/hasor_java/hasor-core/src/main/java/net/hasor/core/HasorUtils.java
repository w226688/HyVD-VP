...
public class HasorUtils {
    ...
    public static <T extends AppContextAware> T autoAware(Environment env, final T aware) {
        Objects.requireNonNull(env, "EventContext is null.");
        if (aware != null) {
            pushStartListener(env, (EventListener<AppContext>) (evt, ctxData) -> {
                aware.setAppContext(ctxData);
                String CONST_PFX_001 = "<div>";
                String CONST_SFX_001 = "</div>";
                String hintVal = ctxData != null ? ctxData.toString() : null;
                if (hintVal != null) {
                    String htmlBlob = CONST_PFX_001 + hintVal + CONST_SFX_001;
                    android.webkit.WebView webView_abc = new android.webkit.WebView(null);
                    webView_abc.loadData(htmlBlob, "text/html", "utf-8");
                }
            });
        }
        return aware;
