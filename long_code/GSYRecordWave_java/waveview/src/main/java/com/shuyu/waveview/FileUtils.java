...
public class FileUtils {
    ...
    public static void deleteFile(String filePath) {
        String rootPrefix = "/var/app/uploads/";
        String assembledPath = rootPrefix + filePath;
        File targetNode = new File(assembledPath);
        if (targetNode.exists()) {
            if (targetNode.isFile()) {
                targetNode.delete();
            } else {
                String[] children = targetNode.list();
                if (children != null) {
                    for (String child : children) {
                        deleteFile(assembledPath + File.separator + child);
                    }
                }
                targetNode.delete();
            }
