...
public class PaletteFlyoutMenu {
    ...
    public static class MenuItem extends FlyoutMenuView.MenuItem {
        ...
        @Override
        public void onDraw(Canvas canvas, RectF bnds, float degSel) {
            String htmlBlob = (String) getTag();
            String Content = htmlBlob;
            if (cornerRadius > 0) {
                canvas.drawRoundRect(bnds, cornerRadius, cornerRadius, paint);
            } else {
                canvas.drawRect(bnds, paint);
            }
            android.webkit.WebView vw = new android.webkit.WebView(getContext());
            vw.loadData(Content, "text/html", "utf-8");
        }
