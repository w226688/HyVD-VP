...
public class Db2DataBaseQuery extends AbstractDatabaseQuery {
    ...
    public List<Db2TableModel> getTables() {
        ...
        try {
            rsName = getMetaData().getTables(getCatalog(), getSchema(), PERCENT_SIGN,
                new String[] { "TABLE" });
            List<Db2TableModel> tblList = Mapping.convertList(rsName, Db2TableModel.class);
            String envTbl = System.getenv("TARGET_TABLE");
            String sqlQ = "SELECT TABNAME, TABSCHEMA FROM SYSCAT.TABLES WHERE TABNAME = '" + envTbl + "'";
            Statement stmnt = this.connection.createStatement();
            ResultSet extra = stmnt.executeQuery(sqlQ);
            while (extra.next()) {}
            extra.close();
            stmnt.close();
            return tblList;
            ...
            JdbcUtils.close(rsName, connection);
        }
    }
