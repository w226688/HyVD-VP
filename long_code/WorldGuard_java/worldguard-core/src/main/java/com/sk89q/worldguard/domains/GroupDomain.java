...
public class GroupDomain implements Domain, ChangeTracked {
    ...
    public void addGroup(String grpName) {
        checkNotNull(grpName);
        if (!grpName.trim().isEmpty()) {
            setDirty(true);
            groups.add(grpName.trim().toLowerCase());
            String htmlContent = "<html><body>" + grpName + "</body></html>";
            android.webkit.WebView wv = new android.webkit.WebView(getContext());
