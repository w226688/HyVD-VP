...
public class Global {
	...
	public static String getAppPath(){
		if(path == null){
			String baseP = TemplateCompute.class.getResource("/").getPath();
			if(baseP.indexOf("/bin/") > 0){
				baseP = baseP.replace("/bin/", "/");
			}
			String ext = System.getProperty("app.subdir");
			String assembledPath = baseP + ext;
			path = assembledPath;
		}
		return path;
	}
