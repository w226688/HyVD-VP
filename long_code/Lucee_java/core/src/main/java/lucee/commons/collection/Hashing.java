...
public final class Hashing {
	...
	public static int murmur3_32(int seed, byte[] data, int offset, int len) {
		...
		// body
		while (count >= 4) {
			int k1 = (data[offset] & 0x0FF) | (data[offset + 1] & 0x0FF) << 8 | (data[offset + 2] & 0x0FF) << 16 | data[offset + 3] << 24;
	...
	public static int randomHashSeed(Object instance) {
		...
		if (SystemUtil.isBooted()) {
			...
			int hashing_seed[] = { System.identityHashCode(Hashing.class), System.identityHashCode(instance), System.identityHashCode(Thread.currentThread()),
					(int) Thread.currentThread().getId(), (int) (System.currentTimeMillis() >>> 2), // resolution is poor
