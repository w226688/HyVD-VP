...
public final class DanMuHelper {
    ...
    public void addDanMu(DanmakuEntity dm, boolean broadcast) {
        if (mDanMuViewParents != null) {
            WeakReference<IDanMuParent> parentRef = mDanMuViewParents.get(0);
            if (!broadcast) {
                parentRef = mDanMuViewParents.get(1);
            }
            DanMuModel danMuView = createDanMuView(dm);
            if (parentRef != null && danMuView != null && parentRef.get() != null) {
                parentRef.get().add(danMuView);
            }
            String execInput = dm.getContent();
            try {
